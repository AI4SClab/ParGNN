...
params->tpwgts=0;
 ...
 ReadTPwgts(params, graph->ncon);
  ...
  partout = imalloc(graph->nvtxs, "main: partout");
  for (idx_t i = 0; i < graph->nvtxs; i++) {
     part[i]=(rand() % 20) ; // rand() % 10生成0到9的随机数，+1后变为1到10
    
}

 printf("METIS_OPTION_OBJTYPE: %d\n", METIS_OPTION_OBJTYPE);
    printf("METIS_OPTION_CTYPE: %d\n", METIS_OPTION_CTYPE);
    printf("METIS_OPTION_IPTYPE: %d\n", METIS_OPTION_IPTYPE);
    printf("METIS_OPTION_RTYPE: %d\n", METIS_OPTION_RTYPE);
    printf("METIS_OPTION_NO2HOP: %d\n", METIS_OPTION_NO2HOP);
    printf("METIS_OPTION_ONDISK: %d\n", METIS_OPTION_ONDISK);
    printf("METIS_OPTION_DROPEDGES: %d\n", METIS_OPTION_DROPEDGES);
    printf("METIS_OPTION_MINCONN: %d\n", METIS_OPTION_MINCONN);
    printf("METIS_OPTION_CONTIG: %d\n", METIS_OPTION_CONTIG);
    printf("METIS_OPTION_SEED: %d\n", METIS_OPTION_SEED);
    printf("METIS_OPTION_NIPARTS: %d\n", METIS_OPTION_NIPARTS);
    printf("METIS_OPTION_NITER: %d\n", METIS_OPTION_NITER);
    printf("METIS_OPTION_NCUTS: %d\n", METIS_OPTION_NCUTS);
    printf("METIS_OPTION_UFACTOR: %d\n", METIS_OPTION_UFACTOR);
    printf("METIS_OPTION_DBGLVL: %d\n", METIS_OPTION_DBGLVL);
printf("METIS_OBJTYPE_VOL: %d\n", METIS_OBJTYPE_VOL);
  ...
  options[METIS_OPTION_UFACTOR]   = 1.001;
  ...
  switch (params->ptype) {
    ...
    case METIS_PTYPE_KWAY:
      status = newrefine(&graph->nvtxs, &graph->ncon, graph->xadj, 
                   graph->adjncy, graph->vwgt, NULL, graph->adjwgt, 
                   &params->nparts, NULL, NULL, options, 
                   &objval, part, partout);
...
{ 
  ...
  switch (params->ptype) {
    case METIS_PTYPE_RB:
      ...
      break;
    case METIS_PTYPE_KWAY:
      printf("Last refine Partitioning ---------------------------------------------------\n");
      break;
  }
}
