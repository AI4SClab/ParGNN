...
/*newpart.c*/
void Gvol(ctrl_t *ctrl, graph_t *graph, idx_t niter, real_t ffactor, idx_t omode);
idx_t IsArticulationNode_2(idx_t i, idx_t *xadj, idx_t *adjncy, idx_t *where, idx_t *bfslvl, idx_t *bfsind, idx_t *bfsmrk);
void KWayVolUpdate_2(ctrl_t *ctrl, graph_t *graph, idx_t v, idx_t from, idx_t to, ipq_t *queue, idx_t *vstatus, idx_t *r_nupd, idx_t *updptr, idx_t *updind, idx_t bndtype, idx_t *vmarker, idx_t *pmarker, idx_t *modind);
void xinGreedy_KWayCutOptimize(ctrl_t *ctrl, graph_t *graph, idx_t niter, real_t ffactor, idx_t omode);
void GKO(ctrl_t *ctrl, graph_t *graph, idx_t niter, real_t ffactor, idx_t omode);
...
void BalanceAndRefineLP_2(ctrl_t *ctrl, graph_t *graph, idx_t nparts, idx_t *where);


int newrefine(idx_t *nvtxs, idx_t *ncon, idx_t *xadj, idx_t *adjncy,idx_t *vwgt, idx_t *vsize, idx_t *adjwgt, idx_t *nparts, 
